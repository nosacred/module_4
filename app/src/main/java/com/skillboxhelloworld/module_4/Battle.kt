...
class Battle(teamA: Team, teamB: Team) {
    ...
    fun fight(teamOne: Team, teamTwo: Team) {
        ...
        for (i in 0 until teamOne.teamList.size) {
            ...
            } else if (teamTwo.teamList[i].isKilled) {
                teamOne.teamList[i].attack(teamTwo.teamList.random())
            }
    ...
    fun getStatus(teamOne: Team, teamTwo: Team): BattleState {
        var team1count = 0
        var team2count = 0
        var team1TotalHealth = 0
        var team2TotalHealth = 0
        teamOne.teamList.forEach{if(!it.isKilled){ team1count++; team1TotalHealth+=it.currentHealth}}
        teamTwo.teamList.forEach {if(!it.isKilled){ team2count++;team2TotalHealth+=it.currentHealth}}
        ...
        return Progress(team1count, team2count, team1TotalHealth, team2TotalHealth)
    }
}
